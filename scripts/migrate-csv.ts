...
import { parseCuisines, mapOccasion, mapVegetarian, splitCities } from "../lib/csv-migrate";
...
function buildLocations(
  city: string,
  locality: string,
): { cities: string[]; locality: string | null } {
...
async function main() {
  ...
  if (cuisinesToUpsert.length) {
    const { error } = await supabase
      .from("cuisines")
      .upsert(cuisinesToUpsert, { onConflict: "name" });
  ...
  console.log(
    `Seeded ${cuisinesToUpsert.length} cuisines; ${fallbackCuisines.length} use fallback emoji.`,
  );
  ...
  for (const { payload } of payloads) {
    ...
    const { error } = await supabase.rpc("upsert_restaurant_with_locations", {
      payload:
        payload as unknown as Database["public"]["Functions"]["upsert_restaurant_with_locations"]["Args"]["payload"],
    });
    if (error) {
