...
const optionalEnum = <T extends readonly [string, ...string[]]>(values: T) =>
  z
    ...
    .transform((v) => (v === '' || v == null ? null : (v as T[number])))

const triState = z
  .union([z.enum(TRI_STATE), z.literal(''), z.null()])
  .transform((v) => (v === '' || v == null ? null : (v as 'yes' | 'no')))
...
export const restaurantSchema = z.object({
  ...
  photo_url: z.string().url().nullable().optional().transform((v) => v || null),
  notes: z.string().trim().nullable().optional().transform((v) => v || null),
  pros: z.string().trim().nullable().optional().transform((v) => v || null),
  cons: z.string().trim().nullable().optional().transform((v) => v || null),
  recommendations: z
    .string()
    .trim()
    .nullable()
