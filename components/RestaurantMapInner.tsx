...

{
  ...
  if (!proto.__diningGuidePatched) {
    ...
    proto.flyTo = function patchedFlyTo(this: PatchedMap, ...args: Parameters<L.Map["flyTo"]>) {
      if (this._diningGuideRemoved) return this;
...
function LocateControl() {
  ...
  useEffect(() => {
    ...
    setContainer(ctl.getContainer() ?? null);
    return () => {
      ctl.remove();
      setContainer(null);
  ...
  return (
    <div className="flex w-72 flex-col gap-2.5 p-4">
      <Link href={`/${marker.slug}`} className="group flex flex-col gap-2.5">
        ...
        {marker.notes ? (
          <p className="text-muted-foreground line-clamp-2 text-sm italic">{marker.notes}</p>
        ) : null}
